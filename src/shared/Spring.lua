...
function Spring.new(initial: (number | Vector3)?, clock: (() -> number)?): Spring
...
function Spring:Impulse(velocity: Vector3 | number)
...
function Spring:_positionVelocity(now: number): number
...
export type Spring = {
	...
	Impulse: (self: Spring, velocity: Vector3 | number) -> (),
	TimeSkipping: (self: Spring, delta: number) -> (),
}
